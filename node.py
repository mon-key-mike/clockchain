...
class SimpleBlock:
    ...
    def calculate_hash(self):
        """Calculate the hash of the block based on its index, timestamp, and previous hash."""
        block_string = f"{self.index}{self.timestamp}{self.previous_hash}".encode()
...
class SimpleBlockchain:
    ...
    def load_genesis_block(self):
        ...
        try:
            with open(self.genesis_file, 'r') as f:      
                data = json.load(f)
                self.chain.append(SimpleBlock(**data))  # Load the genesis block
        ...
        try:
            with open(self.blockchain_file, 'r') as f:   
                ...
                for block in data['chain']:
                    self.chain.append(SimpleBlock(**block))
    ...
    def broadcast_chain(self, peers):
        ...
        for peer in peers:
            try:
                # Prepare the payload
                payload = {'chain': [block.to_dict() for block in self.chain]}
                response = requests.post(f'http://{peer}/chain', json=payload)
